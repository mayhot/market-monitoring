...
- 新增 VS Code 状态栏每日盈亏展示功能：在 VS Code 底部状态栏显示投资组合的今日盈亏金额和收益率，格式为 `$(graph) +1,234.56  +1.23%`。
- 状态栏盈亏支持颜色模式：根据 `marketMonitoring.colorMode` 配置自动切换红涨绿跌或绿涨红跌配色。
- 点击状态栏项可触发行情刷新。
- 新增配置项 `marketMonitoring.showDailyProfitInStatusBar`（默认开启），可控制状态栏盈亏显示。
- 悬停状态栏项可查看详细信息：盈亏金额、收益率和标的数量。
- 新增配置项 `marketMonitoring.autoStart`（默认开启），VS Code 启动后自动开始行情轮询，无需打开插件面板即可在状态栏看到实时盈亏。

### 变更

- 扩展版本由 `0.5.0` 升级至 `0.5.1`。

## [0.5.0] - 2026-08-07

### 修复

- 修复名称列中预警图标压盖名称文字的问题：重新分配名称单元格内部的弹性收缩优先级，名称文本不再被压缩让位给图标；新增 `.alert-badges` 容器统一包裹预警徽章，空间不足时优先裁切徽章区域而非名称内容，保证名称完整展示。

### 变更

- 行情行（`.quote`）的内边距由 `4px 7px` 调整为 `2px`（上下左右均为 2px），压缩行高以在相同可视区域内展示更多行。
- 行情行列与列之间的 grid 间距由 `5px` 调整为 `1px`，进一步压缩水平空白。
- 扩展版本由 `0.4.9` 升级至 `0.5.0`。
...
- 修复 webview 脚本中 `\d` 正则转义导致 ETF 圆点无法显示的问题（改用 `[0-9]` 避免模板字符串转义）。

### 变更

- ETF 圆点大小由 5px 调整为 4px，与持有圆点保持一致。
