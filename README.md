...
- 支持价格和涨跌幅预警。
- 底部右下角展示可配置指数（由 `marketMonitoring.indexSymbol` 指定）的实时行情，悬停可查看全市场涨跌平家数。
- Activity Bar 独立行情视图。
- 支持手动刷新、启动、暂停。
...
状态栏还提供两个可独立开关的展示项，均默认关闭：将 `marketMonitoring.showStatusBar` 设为 `true` 可在状态栏显示前几个标的涨跌的紧凑摘要；将 `marketMonitoring.showGroupDailyProfitBar` 设为 `true` 可在行情面板的指数栏上方显示一行所有分组的「今日盈亏」总额（Σ 当日涨跌额 × 持仓，跨所有分组汇总），悬停可查看各分组明细。
...
行情面板底部右下角展示一个指数的实时行情，由 `marketMonitoring.indexSymbol` 配置项指定（可选 `sh000985` 中证全指、`sh000001` 上证指数、`sz399001` 深证成指、`sz399006` 创业板指、`sh000688` 科创50、`bj899050` 北证50，默认 `sh000001` 上证指数）。悬停指数报价可查看全市场上涨、下跌、平盘家数。指数行情由扩展自动拉取，不需要添加到 `marketMonitoring.symbols`。
